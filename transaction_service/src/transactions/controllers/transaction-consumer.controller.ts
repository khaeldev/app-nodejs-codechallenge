...
export class TransactionConsumerController {
  ...
  async handleTransactionValidated(
    ...
    try {
      ...
      this.logger.error(
        `Failed to parse message payload or invalid structure: ${(error as Error).message}`,
      );
      throw new Error('Invalid message payload structure');
    }
    ...
    try {
      ...
      this.logger.error(
        `Error processing transaction ${payload.transactionExternalId}: ${(error as Error).message}`);
    }
  }
